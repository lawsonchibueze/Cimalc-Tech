...
const copy: Record<AuthMode, { title: string; intro: string; submit: string }> =
  {
    signin: {
      title: "Welcome back",
      intro:
        "Save quotes, track requests, and manage your Cimalc Tech experience.",
      submit: "Sign in",
    },
    signup: {
    ...
    forgot: {
      ...
      intro: "We’ll send a secure reset link to your email.",
      submit: "Send reset link",
    },
    reset: {
      title: "Choose a new password",
      intro: "Pick a password you have not used elsewhere.",
      submit: "Save new password",
    },
  };

const linkClass =
  "font-medium text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand";
...
function oauthMessage(code: string | null) {
  ...
  const known: Record<string, string> = {
    account_not_linked:
      "This email already has an account that has not been verified, so Google cannot be linked yet. Sign in with your password, or contact us for help.",
    state_mismatch:
      "Your browser blocked part of the Google sign-in. Try again, and if it repeats, allow cookies for this site.",
    state_security_mismatch:
      "Your browser blocked part of the Google sign-in. Try again, and if it repeats, allow cookies for this site.",
    please_restart_the_process: "The Google sign-in expired. Please try again.",
    access_denied:
      "Google sign-in was cancelled, or this Google account is not allowed to use it.",
  };
  ...
  function validate(): string | null {
    ...
    if (mode === "signup" || mode === "reset") {
      ...
      if (!passwordResult.success)
        return passwordResult.error.issues[0].message;
    }
    ...
    >
      <div className="mb-8">
        <Link
          ...
          className="mb-5 inline-flex w-fit focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
        >
      ...
      {sentTo ? (
        <div
          role="status"
          className="flex flex-col items-center gap-3 rounded-md bg-background p-6 text-center"
        >
          <MailCheck className="h-8 w-8 text-success" aria-hidden="true" />
          <p className="text-sm leading-6 text-default">
        ...
        <div
          role="alert"
          className="space-y-4 rounded-md bg-error-bg p-4 text-sm text-error"
        >
      ...
      <div className="mt-6 text-center text-sm text-muted">
        {mode === "signup" && (
          <>
            ...
            <Link href={withRedirect("/auth/sign-in")} className={linkClass}>
              Sign in
            </Link>
          </>
        )}
        {mode === "signin" && (
          <>
            New to Cimalc Tech?{" "}
            <Link href={withRedirect("/auth/sign-up")} className={linkClass}>
              Create an account
            </Link>
            <br />
            <Link
              href="/auth/forgot-password"
              className="mt-3 inline-block text-xs text-muted hover:text-brand"
            >
              Forgot password?
            </Link>
          </>
        )}
        {(mode === "forgot" || mode === "reset") && !sentTo && (
          <Link href="/auth/sign-in" className={linkClass}>
            Back to sign in
          </Link>
        )}
