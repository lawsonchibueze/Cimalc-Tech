...
export default function AdminQuoteDetailPage({
  ...
  const quote = useQuery({
    queryKey: quoteKeys.adminDetail(id),
  ...
  const changeStatus = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      setConfirming(null);
      toast.success(
        `Marked as ${QUOTE_STATUS_META[updated.status].label.toLowerCase()}`,
      );
    },
    onError: (error) => {
      setConfirming(null);
  ...
  const reply = useMutation({
    ...
    onError: (error) =>
      toast.error(errorMessage(error, "Your reply could not be sent.")),
  });
  const respond = useMutation({
    mutationFn: (data: QuoteResponseInput) => respondToAdminQuote(id, data),
    onSuccess: (updated) => {
      store(updated);
      toast.success("Quote sent to the customer");
    },
    onError: (error) =>
      toast.error(errorMessage(error, "The quote could not be sent.")),
  });
  ...
  if (quote.isError || !quote.data) {
    ...
    return (
      <ErrorState
        title={notFound ? "Quote not found" : undefined}
        description={notFound ? "This quote no longer exists." : undefined}
        onRetry={notFound ? undefined : () => void quote.refetch()}
      />
    );
  }
          ...
          {actions.length === 0 ? (
            <p className="text-sm text-muted">
              {data.status === "CANCELLED"
                ? "The customer cancelled this request, so its status is final."
              ...
              {actions.map((action) => (
                <Button
                  ...
                  isLoading={
                    ...
                    changeStatus.variables === action.status
                  }
                  disabled={changeStatus.isPending}
                  onClick={() =>
                    action.destructive
                      ? setConfirming(action)
                      : changeStatus.mutate(action.status)
                  }
                >
                  {action.label}
                </Button>
        ...
        <Card>
          <CardContent className="space-y-5 p-5">
            <div>
              ...
              <p className="mt-2 text-sm text-muted">
                Set the price and availability for each product. Sending it
                emails the customer and shows the details on their dashboard.
              </p>
            </div>
            <QuoteResponseForm
              ...
              onSubmit={(values) => respond.mutateAsync(values)}
            />
          </CardContent>
        </Card>
      )}
          ...
          <QuoteThread
            ...
            closedReason={
              CLOSED_STATUSES.includes(data.status)
                ? "This request is closed, so replies are turned off."
                : undefined
            }
        ...
        <Card className="h-fit">
          <CardContent className="space-y-4 p-5">
            ...
            <a
              ...
              className="flex items-center gap-2 break-all text-sm text-brand"
            >
      ...
      >
        <div className="space-y-5">
          <p className="text-sm leading-6 text-muted">
            The customer is emailed about this change. You can reopen the
            request for review afterwards.
          </p>
          <div className="flex justify-end gap-3">
            ...
            <Button
              ...
              onClick={() =>
                confirming && changeStatus.mutate(confirming.status)
              }
            >
              {confirming?.label}
            </Button>
