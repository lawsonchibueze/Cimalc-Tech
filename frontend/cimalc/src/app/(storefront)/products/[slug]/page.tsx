...
function VariantRow({
  ...
  return (
    <div
      className={`flex flex-wrap items-center justify-between gap-3 px-4 py-4 text-sm ${first ? "" : "border-t border-border"}`}
    >
      <div>
      ...
      <Badge variant={variant.availability ? "success" : "neutral"}>
    ...
    return (
      <div className="mx-auto max-w-[1440px] px-4 py-8 md:px-8">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-[1fr_420px]">
          <Skeleton className="mx-auto aspect-square w-full max-w-[380px]" />
    ...
    <div className="mx-auto max-w-[1440px] px-4 py-8 md:px-8">
      ...
      <div className="mt-6 grid grid-cols-1 gap-10 md:grid-cols-[1fr_420px]">
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href={`/categories/${product.categorySlug}`}
              className="text-xs font-semibold uppercase tracking-[0.14em] text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
            >
          ...
          <div className="rounded-md border border-brand/15 bg-brand/[0.04] p-4">
            ...
            <p className="mt-1 text-sm leading-5 text-muted">
              Tell us what you need and we’ll confirm current availability and
          ...
          <p className="text-xs text-muted">
            {product.inStock
              ? "No payment is required to request a quote."
              : "Out of stock right now. You can still request a quote and we’ll confirm when it can be sourced."}
          </p>
        </div>
      </div>
      {product.variants.length > 0 && (
        <section className="mt-16 max-w-3xl">
          ...
          <h2 className="mt-2 text-2xl font-bold tracking-tight text-default">
            Available options
          </h2>
      ...
      {related && related.length > 0 && (
        <section className="mt-16">
          <h2 className="mb-6 text-xl font-bold text-default">
            Related Products
          </h2>
      ...
      <section className="mt-16 rounded-md bg-brand p-6 text-white md:flex md:items-center md:justify-between md:gap-8 md:p-10">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-white/70">
            Tell us about your use case, quantity, or budget and our team can
            point you toward the right option.
          </p>
        </div>
        <Button
          href="/contact"
