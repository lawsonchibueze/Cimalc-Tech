...
export function cancelQuote(id: string) {
  return apiRequest<Quote>(`/quotes/${encodeURIComponent(id)}/cancel`, {
    method: "PATCH",
  });
}

export function addQuoteMessage(id: string, body: string) {
  return apiRequest<Quote>(`/quotes/${encodeURIComponent(id)}/messages`, {
    method: "POST",
...
export type AdminQuotePage = Paginated<
  Quote,
  PageMeta & { counts: Partial<Record<QuoteStatus, number>> }
>;

export function getAdminQuotes(params: AdminQuoteQuery = {}) {
  return apiRequest<AdminQuotePage>(
    `/admin/quotes${toQueryString({ ...params })}`,
  );
}
...
export function updateAdminQuoteStatus(id: string, status: QuoteStatus) {
  return apiRequest<Quote>(`/admin/quotes/${encodeURIComponent(id)}/status`, {
    method: "PATCH",
...
export function addAdminQuoteMessage(id: string, body: string) {
  return apiRequest<Quote>(`/admin/quotes/${encodeURIComponent(id)}/messages`, {
    method: "POST",
    body: JSON.stringify({ body }),
  });
}

export interface QuoteResponseLineInput {
  id: string;
  unitPrice: number;
...
export interface QuoteResponseInput {
  lines: QuoteResponseLineInput[];
  message?: string;
}

/** Staff answer a request with a price and availability for each product. */
export function respondToAdminQuote(id: string, data: QuoteResponseInput) {
  return apiRequest<Quote>(`/admin/quotes/${encodeURIComponent(id)}/respond`, {
    method: "POST",
