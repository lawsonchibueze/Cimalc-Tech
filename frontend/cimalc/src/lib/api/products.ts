...
function mapProduct(item: BackendProduct): Product { const category = item.category ?? {}; const images = Array.isArray(item.images) ? item.images : item.image ? [{ id: String(item.id) + "-image", url: item.image, alt: item.name }] : [{ id: String(item.id) + "-placeholder", url: "/products/mock.png", alt: item.name }]; return { id: String(item.id), slug: item.slug ?? toSlug(item.name), name: item.name, price: Number(item.price ?? 0), description: item.description ?? "", categorySlug: category.slug ?? category.id ?? String(item.categoryId ?? ""), images: images.map((image: BackendProduct, index: number) => ({ id: String(image.id ?? index), url: image.url ?? image.publicUrl, alt: image.alt ?? item.name, storageKey: image.storageKey ?? image.key })), inStock: item.inStock !== undefined ? Boolean(item.inStock) : item.availability === "IN_STOCK" || Number(item.stock ?? 0) > 0, specifications: item.variants ?? item.specifications }; }
