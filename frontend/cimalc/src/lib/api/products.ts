...
export async function getProducts(params: ProductQuery = {}) {
  return mapPage(
    await apiRequest<BackendPage>(`/products${toQueryString({ ...params })}`),
  );
}

/** The featured section on the home page shows up to eight products. */
export async function getFeaturedProducts(limit = 8) {
  return mapPage(
    await apiRequest<BackendPage>(
      `/products/featured${toQueryString({ limit })}`,
    ),
  ).data;
}
...
export async function getNewArrivals(limit = 4) {
  return mapPage(
    await apiRequest<BackendPage>(
      `/products/new-arrivals${toQueryString({ limit })}`,
    ),
  ).data;
}
...
export async function getProductBySlug(slug: string) {
  return mapProduct(
    await apiRequest<BackendProduct>(`/products/${encodeURIComponent(slug)}`),
  );
}

export function getProductsByCategory(slug: string, params: ProductQuery = {}) {
  return apiRequest<BackendPage>(
    `/categories/${encodeURIComponent(slug)}/products${toQueryString({ ...params })}`,
  ).then(mapPage);
}
...
export async function getProductById(id: string) {
  return mapProduct(
    await apiRequest<BackendProduct>(
...
export async function createProduct(input: ProductInput) {
  return mapProduct(
    await apiRequest<BackendProduct>("/admin/products", {
      method: "POST",
      body: JSON.stringify(toBackendProduct(input)),
    }),
  );
}
...
export async function updateProduct(id: string, input: Partial<ProductInput>) {
  return mapProduct(
    await apiRequest<BackendProduct>(
      `/admin/products/${encodeURIComponent(id)}`,
      { method: "PATCH", body: JSON.stringify(toBackendProduct(input)) },
    ),
  );
}

export async function deleteProduct(id: string) {
  await apiRequest<unknown>(`/admin/products/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
}
