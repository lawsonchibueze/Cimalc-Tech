...
function setup() {
  const prisma = {
    product: {
      findFirst: vi.fn().mockResolvedValue({ id: 'p1', name: 'Phone' }),
    },
    quote: {
      ...
      update: vi
        .fn()
        .mockImplementation(async () => quoteRow({ status: 'CANCELLED' })),
    },
    quoteMessage: { create: vi.fn() },
  };
  const mail = { send: vi.fn(), staffAddress: undefined };
  return {
    service: new QuotesService(prisma as never, mail as never),
    prisma,
    mail,
  };
}
...
describe('QuotesService', () => {
  it('only accepts quotes for published products', async () => {
    ...
    await expect(
      service.create(
        { customerName: 'Ada', email: 'a@b.co', productId: 'p1', quantity: 1 },
        undefined,
      ),
  ...
  it('exposes the original message, message senders and a slim product', async () => {
    ...
    const quote = await service.create(
      { customerName: 'Ada', email: 'a@b.co', productId: 'p1', quantity: 2 },
      viewer,
    );
    expect(quote.message).toBe('Need two');
    expect(quote.messages.map((message) => message.sender)).toEqual([
      'CUSTOMER',
      'STAFF',
    ]);
    expect(quote.productLines[0].product).toEqual({
      id: 'p1',
      slug: 'phone',
      name: 'Phone',
  ...
  it('matches guest quotes by email even when the account is not verified', async () => {
    const { service, prisma } = setup();
    await service.findMine(viewer);
    expect(
      JSON.stringify(prisma.quote.findMany.mock.calls[0][0].where),
    ).toContain('"userId":null');

    // Email verification is not configured, so hiding guest quotes until then
    // stopped customers from seeing their own request after creating an account.
    await service.findMine({ ...viewer, emailVerified: false });
    expect(
      JSON.stringify(prisma.quote.findMany.mock.calls[1][0].where),
    ).toContain('"userId":null');
  });
  ...
  it('refuses to cancel a quote that was already answered', async () => {
    ...
    await expect(service.cancel(viewer, 'q1')).rejects.toBeInstanceOf(
      ConflictException,
    );
  ...
  it('cancels a pending quote and records the time', async () => {
    ...
    expect(prisma.quote.update.mock.calls[0][0].data).toMatchObject({
      status: 'CANCELLED',
  ...
  it('does not allow messages on a closed quote', async () => {
    ...
    await expect(
      service.addMessage(viewer, 'q1', { body: 'hello' }),
